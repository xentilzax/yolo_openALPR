...
int (*print_to_stdout)(const char* str, ...) = printf;
int (*print_to_stderr)(FILE* stream, const char* str, ...) = fprintf;
...
void pm(int M, int N, float *A)
{
    int i,j;
    for(i =0 ; i < M; ++i){
        print_to_stdout("%d ", i+1);
        for(j = 0; j < N; ++j){
            print_to_stdout("%2.4f, ", A[i*N+j]);
        }
        print_to_stdout("\n");
    }
    print_to_stdout("\n");
...
void find_replace_extension(const char *str, const char *orig, const char *rep, char *output)
{
    char *buffer = (char*)calloc(8192, sizeof(char));
...
void malloc_error()
{
//    print_to_stderr(stderr, "Malloc error\n");
    throw std::runtime_error("Malloc error");
}

void file_error(const char *s)
{
//    print_to_stderr(stderr, "Couldn't open file: %s\n", s);
    throw std::runtime_error("Couldn't open file: " + std::string(s));
}
...
void print_statistics(float *a, int n)
{
    ...
    print_to_stdout("MSE: %.6f, Mean: %.6f, Variance: %.6f\n", mse_array(a, n), m, v);
}
